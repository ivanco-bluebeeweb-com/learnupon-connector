"""Connection lifecycle for LearnUpon Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from learnupon_client import LearnUponClient
...
@chat.function(
    "connect_learnupon",
    "Connect LearnUpon account via credentials.",
    action_type="write",
    chain_callable=True,
    event="learnupon-connector.connect_learnupon",
...
async def connect_learnupon(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    ...
    record = {
        "id": cid,
        "label": params.label or "LearnUpon Account",
...
@chat.function(
    "list_connections",
    "List connected LearnUpon accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_learnupon",
    "Disconnect LearnUpon account.",
    action_type="write",
    chain_callable=True,
    event="learnupon-connector.disconnect_learnupon",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_learnupon(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
